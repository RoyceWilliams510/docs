---
title: "Stripe Practice Plan Subscription Functions"
...
<Step title="Return Data">
  ...
  <ul>
    <li>For trials: subscription ID and setup intent client secret</li>
    <li>For regular: subscription ID and payment intent client secret</li>
  </ul>
</Step>
</Steps>
...
<Steps>
  <Step title="Authentication Check">Ensure the user is authenticated.</Step>
  ...
  <Step title="Stripe Customer Handling">
    <AccordionGroup>
      <Accordion title="No Stripe Customer ID">
        <ul>
          <li>Create a new Stripe customer.</li>
          <li>Update Firestore with the new customer ID.</li>
          <li>
            Return:{" "}
            <code>
              {" "}
              hasSubscription: false, message: "No active subscription found", hasTrial:
              false{" "}
            </code>
          </li>
        </ul>
      ...
      <Accordion title="Existing Stripe Customer ID">
        <ul>
          <li>
            Call <code>checkUserSubscription</code> with customer ID and
            required plan ID.
          </li>
          <li>Return subscription status.</li>
        </ul>
      </Accordion>
    </AccordionGroup>
  </Step>
  <Step title="checkUserSubscription Helper">
    <ul>
      <li>Retrieve customer from Stripe.</li>
      <li>Check for active and trial subscriptions.</li>
      <li>Return detailed subscription status object.</li>
    </ul>
  </Step>
</Steps>
...
<Step title="Subscription Lookup">
  List Stripe subscriptions for the customer with status <code>trialing</code>.
  Find the subscription matching the practice plan product.
</Step>
...
<AccordionGroup>
  <Accordion title="Key Features">
    <ul>
      <li>Immediately cancels trial subscription</li>
      <li>Updates both Stripe and Firestore records</li>
      <li>Handles error cases (no subscription found, invalid customer)</li>
      <li>No prorating of charges</li>
      <li>Forces immediate invoice generation</li>
    </ul>
  </Accordion>
  <Accordion title="Error Handling">
    <ul>
      <li>Checks for user existence</li>
      <li>Validates subscription existence</li>
      <li>Wraps all operations in try/catch</li>
...
<Check>
  You now have a reusable, well-structured MDX boilerplate for documenting
  Stripe-related backend functions. Adapt the sections and components as needed
